...
При запуске скрипта с папкой изображений `data/Уши` и GT-таблицей `data/gt_converted_test.csv` и весами `data/weights/best_ensemble_model33.pth` на выходе должны получиться метрики:  
...
![Диаграмма компонентов](figs/1.png)
...
### Диаграмма объектов
![Диаграмма объектов](figs/2.png)
### Таблица объектов

| Объект | Назначение | Комментарий |
| --- | --- | --- |
| `model: EnsembleModel(beit3_large_patch16_224)` | Вычисление логитов/вероятностей по батчу. | Для инференса грузятся веса `best_ensemble_model*.pth`; устройство `cuda`/`cpu`. |
| `train_tf`, `val_tf` | Преобразования изображений. | Собраны через `torchvision.transforms.Compose`. |
| `train_ds: EarDataset` | Датасет обучения. | Читает Excel, формирует one-hot, фильтрует «качество=нет». |
| `val_ds: EarDataset` | Датасет валидации. | Аналогично `train_ds`, но с валид. трансформами. |
| `train_dl`, `val_dl` | Даталоадеры. | Батч-размеры: train=2, val=1 (по умолчанию). |
| `criterion: BCEWithLogitsLoss` | Функция потерь для multi-label. | Использует рассчитанный `pos_weight`. |
| `optimizer: AdamW` | Обучение модели. | `lr=5e-6`, `weight_decay=1e-4`. |
| `infer_ds: DirByListDataset` | Датасет для инференса по списку из GT. | Строит список путей к файлам `study_id`. |
| `thresholds` | Пороговая таблица классов. | Базово `0.5`; для `OP/ONP` может тюниться по `r_min/p_min`. |
| `postprocess_cfg` | Настройки пост-обработки. | `norm_veto_th`, `sulfur_veto_th`, `ng_margin`. |

### Диаграмма коммуникации  
![Диаграмма коммуникации](figs/3.png)

### Таблица коммуникации объектов

| №  | Вызываемая функция                          | Назначение                           | Комментарий                                                     |
| -- | ------------------------------------------- | ------------------------------------ | --------------------------------------------------------------- |
| 1  | `pd.read_csv / read_excel`                  | Загрузка GT-таблицы.                 | Ожидаются столбцы: `study_id,NORM,OP,ONP,CI,OTH`.               |
| 2  | `DirByListDataset(...)`                     | Подготовка списка изображений по GT. | Имена выравниваются по найденным файлам.                        |
| 3  | `DataLoader(...)`                           | Батч-чтение изображений.             | Без shuffle, детерминированный порядок.                         |
| 4  | `load_state_dict(...)`                      | Загрузка весов модели.               | Поддержка `module.*` и `{"state_dict":...}`.                    |
| 5  | `model.forward(x)`                          | Получение `avg_logits`.              | Ансамбль усредняет логиты.                                      |
| 6  | `torch.sigmoid(avg_logits)`                 | Перевод в вероятности.               | Multi-label настройка.                                          |
| 7  | `pick_threshold_recall_then_precision(...)` | (Опц.) тюнинг порогов `OP/ONP`.      | Цель: `Recall ≥ r_min` и `Precision ≥ p_min`.                   |
| 8  | Пост-обработка                              | «Вето» и приоритет `ONP>OP`.         | `NORM` и `CI` глушат `OP`; `ONP` может глушить `OP` при зазоре. |
| 9  | `to_csv(preds.csv)`                         | Сохранение бинарных предсказаний.    | Столбцы: `study_id,n,op,onp,ci,oth`.                            |
| 10 | `sklearn.metrics.*`                         | Подсчёт метрик per-class.            | Precision/Recall/F1/Specificity/Accuracy.                       |
| 11 | `to_csv(metrics.csv)`                       | (Опц.) Сохранение метрик.            | Порог на класс включён.                                         |


### Диаграмма и описание математической модели  

![Диаграмма математической модели](figs/4.png)

### Таблица математической модели

| Название модели/этап                | Вызываемая функция                                  | Назначение                          | Комментарий                                            |
| ----------------------------------- | --------------------------------------------------- | ----------------------------------- | ------------------------------------------------------ |
| Входные изображения (`RGB 224×224`) | —                                                   | Источник данных                     | Перед инференсом приводятся к размеру 224×224.         |
| Предобработка                       | `transforms.Resize/ToTensor/Normalize`              | Нормализация входа                  | ImageNet mean/std.                                     |
| Бэкбон (timm)                       | `timm.create_model("beit3_large_patch16_224", ...)` | Экстракция признаков и логитов      | В проекте используется один бэкбон в ансамбле.         |
| Усреднение логитов                  | `avg_logits = mean(logits_list)`                    | Агрегированный логит ансамбля       | Уменьшает шум отдельных моделей.                       |
| Вероятности                         | `sigmoid(avg_logits)`                               | Перевод логитов в вероятности       | Multi-label.                                           |
| Пороговая фильтрация                | `probs >= thresholds`                               | Бинаризация по классам              | Для `OP/ONP` пороги можно тюнить.                      |
| Пост-обработка                      | «veto(NORM, CI)», `ONP>OP`                          | Снижение FP и разрешение конфликтов | Пороги: `norm_veto_th`, `sulfur_veto_th`, `ng_margin`. |
| Выходы                              | `preds.csv`, `metrics.csv`, `infer_json()`          | Сохранение результатов              | CSV для предиктов и метрик; JSON для внешнего API.     |
